...
)
{
	...
	for (i = 0; i < (o_ol_cnt - 1); i++) {
		tmp = (MAXITEMS + 1) * supware[ol_num_seq[i]] +
		      itemid[ol_num_seq[i]];
		min_num = i;
		for (j = i + 1; j < o_ol_cnt; j++) {
			if ((MAXITEMS + 1) * supware[ol_num_seq[j]] +
				    itemid[ol_num_seq[j]] <
			    tmp) {
				tmp = (MAXITEMS + 1) * supware[ol_num_seq[j]] +
				      itemid[ol_num_seq[j]];
				min_num = j;
			}
		}
	...
	for (ol_number = 1; ol_number <= o_ol_cnt; ol_number++) {
		...
		sqlite3_bind_text(sqlite_stmt, 9, ol_dist_info, -1,
				  SQLITE_STATIC);

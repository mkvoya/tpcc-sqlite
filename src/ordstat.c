...
)
{
	...
	if (byname) {
		...
		for (n = 0; n < namecnt / 2; n++) {
			...
			if (ret != SQLITE_DONE) {
				...
				strcpy(c_first,
				       sqlite3_column_text(sqlite_stmt, 1));
				strcpy(c_middle,
				       sqlite3_column_text(sqlite_stmt, 2));
				strcpy(c_last,
				       sqlite3_column_text(sqlite_stmt, 3));
			}
		}
	...
	for (;;) {
		...
		if (ret == SQLITE_ROW) {
			...
			if (bytes)
				strcpy(ol_delivery_d,
				       sqlite3_column_text(sqlite_stmt, 4));
		} else
			goto sqlerr;
	}
